<template>
    <ul>
        <li v-for="n in 6" :key="n">
            <div>{{ n }}</div>
            <div :class="`h-[25%] w-[50%] absolute ${style.playerAreaDirection[n-1]} ${style.playerAreaXPosition[n-1]} ${style.playerAreaYPosition[n-1]}`">
                ...
                <div :class="`playerCard ${style.card} h-[5.5vh] w-[4vh] bottom-[12%] left-[12%]`"></div>
                <div :class="`playerCard ${style.card} h-[5.5vh] w-[4vh] bottom-[12%] left-[22%]`"></div>
                <div>
                ...
                <div :class="`absolute bottom-[50%] left-[50%] px-[2vw] ${style.chipDisplay}`" v-if="state.betTotalList[0] !== 0">{{ state.betTotalList[0] }}</div>
                <div :class="`absolute bottom-[12%] md:bottom-[10%] left-[75%] md:left-[50%] ${style.chipDisplay}`" >{{ state.stackList[0] }}</div>
            </div>
        </li>
    </ul>
</template>
